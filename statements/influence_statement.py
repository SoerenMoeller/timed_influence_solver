...
class IStatement(RStatement):
    ...
    def __str__(self):
        return f"IStatement({self.start}, {self.end}, {self.lower}, {self.upper}, {self.min_slope}, {self.max_slope})"
    ...
    def create(cls, st: tuple[str, tuple, tuple, tuple, str]):
        return IStatement(st[1][0], st[1][1], st[2][0], st[2][1], st[3][0], st[3][1])
